...
void UserTracker::PeriodicMaintenance(uint64_t xuid,
  ...
  const bool updated_presence_string =
      kernel_state()->presence_manager()->UpdateLocalPresence(user->xuid());
  ...
  if (!(iteration_count % 3)) {
    kernel_state()->presence_manager()->SyncPresence(xuid);
  }

  if (updated_presence_string) {
    kernel_state()->presence_manager()->UpdateXboxLiveLocalUsersPresence(
        {user->xuid()});
  }
