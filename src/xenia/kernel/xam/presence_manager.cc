...
void PresenceManager::Initalize(const uint32_t max_subscriptions) {
  ...
  // Request presence information for friends?
}
...
void PresenceManager::SyncPresence(uint64_t xuid) const {
  ...
  if (presence_synced.GetSyncState().subscribers) {
    kernel_state_->BroadcastNotification(kXNotificationLivePresenceChanged, 0);
  }
}
